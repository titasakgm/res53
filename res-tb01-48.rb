...
def getResInfo(pcode,year)
  ...
  sql = "SELECT r_pak,r_khet,r_pname FROM resinfo "
...
def getOffInfo(hcode,year)
  ...
  sql = "SELECT o_name,o_minisid,m_desc,office.o_provid,prov.o_province FROM office,minisid,prov "
  sql += "WHERE o_code='#{hcode}' AND o_minisid=m_code AND office.o_provid=prov.o_provid"
  res = con.exec(sql)
  con.close
  hn = mn = md = pc = pn = nil
  res.each do |rec|
...
res.each do |rec|
  ...
  #1 get pak khet pname pop2548 from resinfo
