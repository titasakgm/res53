...
<body text='purple'>
<center>
<h4>REPORT 2: รายละเอียดการบันทึกข้อมูล#{amphoe} จังหวัด#{province}<p>
<font size=3><i><b>#{today}</b></i></font></h4>
<input type='button' value='กลับหน้าที่แล้ว' onclick='history.back()' />
<p>
<table width='90%' border='1'>
<tr>
  <th>ลำดับ</th><th>รหัส</th><th>อำเภอ</th><th>หน่วยงาน</th><th>F1</th><th>F2</th><th>F3</th>
  <th>F4</th><th>F5</th><th>F6</th><th>F7</th><th>F8</th><th>วันรายงาน</th>
</tr>
EOF

n = 0
f1bg = f2bg = f3bg = f4bg = f5bg = f6bg = f7bg = f8bg = '#FFFFFF'
res.each do |rec|
  hc = rec[0].to_s
  ...
  f7 = '&#8730;' if (f7 == 'X')
  f8 = '&#8730;' if (f8 == 'X')
  amphoe = getAmpNameFromHcode(hc)
  n += 1
  print "<tr><th>#{n}</th><th>#{hc}</th><td>#{amphoe}</td>"
...
province = getProvName(id)

con = PGconn.connect("localhost",5432,nil,nil,"resource53")
sql = "SELECT * FROM reportmon WHERE pcode='#{id.to_s}' "
sql += "ORDER BY hcode"
res = con.exec(sql)
con.close
