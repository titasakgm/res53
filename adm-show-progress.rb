...
c = CGI::new
user = c['user']
if (user.length == 2) # Province Admin
  print "Location:adm-prov-progress.rb?user=#{user}\n\n"
end
...
res1 = con.exec(sql)
sql = "SELECT provid,province,sum(total),sum(balance) FROM report2 "
...
<tr>
  <th align='left'>รวมทั้งประเทศ</th>
...
res2.each do |rec|
  ...
  pct = '0%' if tot == bal
  if (n%3 == 1)
    print "<tr><td>#{pn}</td><th><span class='progressBar' id='#{pc}'>#{pct}</span></th>"
  elsif (n%3 == 2)
    print "<td>#{pn}</td><th><span class='progressBar' id='#{pc}'>#{pct}</span></th>"
  elsif (n%3 == 0)
    print "<td>#{pn}</td><th><span class='progressBar' id='#{pc}'>#{pct}</span></th></tr>\n"
  end
end
